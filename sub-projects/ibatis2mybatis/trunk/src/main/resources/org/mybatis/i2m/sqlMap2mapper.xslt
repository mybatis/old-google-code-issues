...
<xsl:stylesheet version="1.0"
            ...
            <xsl:if test="@cacheModelsEnabled">
                <xsl:element name="setting">
                    ...
                    <xsl:attribute name="value"><xsl:value-of
                        select="@cacheModelsEnabled" /></xsl:attribute>
                </xsl:element>
            </xsl:if>
    ...
    <xsl:template match="/sqlMap/typeAlias|statement|cacheModel|dynamic|isPropertyAvailable|isNotPropertyAvailable|isNotParameterPresent|isParameterPresent|/sqlMapConfig/properties">
        <xsl:comment>Sorry, I can't migrate <xsl:value-of select="@*" />, see console output for further details.</xsl:comment>
        <xsl:message>Sorry, I can't migrate: <xsl:copy-of select="." />.
        </xsl:message>
    </xsl:template>
